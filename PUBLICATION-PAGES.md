...
content/thumbnail-picks.json which page is each publication's index cover
...
python3 scripts/make_thumbnails.py --audit     # which PDFs look like publisher cover sheets
python3 scripts/make_thumbnails.py --contact   # one sheet of every cover, to eyeball
python3 scripts/make_thumbnails.py [<id> ...]  # build covers from thumbnail-picks.json
...
The publications index is a grid of covers, each the publication's own first
page, sorted newest first. These are a different thing from the evidence figure
on a publication page and live in a different folder:

| | Figure | Cover |
| --- | --- | --- |
| Shown on | the publication page | the index grid |
| Chosen in | `content/figure-picks.json` | `content/thumbnail-picks.json` |
| Written to | `docs/publications/figures/` | `docs/publications/covers/` |
| Licence | recorded per figure in the YAML | recorded once, for all of them |
| Alt text | required, describes the figure | deliberately empty |

Keep covers out of `figures/`. `extract_figures.py` writes `figures/<id>.png`
and `rebuild_figures.py` sweeps `figures/<id>.*` before every rebuild, so a
cover living there would be deleted by the next figure rebuild, silently.
...
python3 scripts/make_thumbnails.py --contact    # look at every cover in one sheet
```

`--audit` proposes, it does not decide: a sparse first page is sometimes a
graphical abstract, which makes the best cover of all. Record the page in
`thumbnail-picks.json`, then build. A publication with no pick falls back to a
title page set in the site's own type, so dropping a reprint into `pdfs/` and
rerunning the script upgrades it later with no code change.

Note that covers can only be regenerated by someone holding the gitignored
`pdfs/` library. Rebuild them per id when a pick changes rather than wholesale —
libwebp version drift makes identical inputs produce byte-different files.
...
sheet, blank, or the wrong document. Nothing but your eye catches that.
