...
Answers the question "which papers do we have enough material to write a page
for?" before any page is written. A paper page needs a verifiable key finding,
...
# Pages whose citations we survey. The generated papers section is skipped —
# its DOIs come from content/papers/ and are already accounted for.
SKIP_PAGES = {"index.html", "papers.html", "about.html", "what-we-do.html", "who-we-are.html"}
...
# papers here are CC-BY but still unreachable.
...
def tier(rec: dict) -> str:
    """How much of a page can we responsibly write for this paper?"""
    if rec.get("error"):
...
TIER_NOTE = {
    "has-page": "Already published as a paper page.",
    ...
    "blocked-no-abstract": "No abstract reachable anywhere — a verifiable key finding cannot be written without the paper itself.",
...
def markdown(records: list[dict]) -> str:
    ...
    lines = [
        "# Paper page status",
        "",
        "Generated by `python3 scripts/survey_papers.py --markdown` — do not edit by hand.",
        "One row per DOI cited anywhere on ciunit.org.",
        "",
        "A paper page is only worth publishing if it states a **verifiable key finding**",
        "(see `CLAUDE.md`, \"Paper pages and GEO\"). In practice that means we need a",
        ...
        f"- **{c['blocked-no-abstract']}** are blocked pending the paper itself",
        "",
        "| Tier | Count | Meaning |",
        "| --- | ---: | --- |",
    ]
...
def main() -> int:
    if "--from-cache" in sys.argv or "--markdown" in sys.argv:
        ...
        # Which papers have a page is the one thing that changes without the
