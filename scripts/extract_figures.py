...
def pdf_for(paper: dict, mapping: dict) -> Path | None:
    ...
    if not entry:
        return None
    # Prefer the largest file when a paper appears more than once — usually the
    # full typeset article rather than an accepted manuscript.
    files = [PDF_DIR / f for f in entry["files"]]
...
def find_figures(doc) -> list[dict]:
    ...
    for pno in range(doc.page_count):
        ...
        for b in blocks:
            ...
            texts = [r for r in (pymupdf.Rect(b2[:4]) for b2 in blocks
                                 if b2 is not b and len((b2[4] or "").split()) > 12)
                     if lo <= (r.x0 + r.x1) / 2 <= hi]
            ...
            clip = max(pad, cap.width * 0.45)
            region.x0 = max(region.x0, cap.x0 - clip)
            region.x1 = min(region.x1, cap.x1 + clip)
...
def extract(paper_id: str, num: int, papers: dict, mapping: dict, dpi: int = 300) -> int:
    paper = papers[paper_id]
    path = pdf_for(paper, mapping)
    if not path:
        print(f"no PDF for {paper_id}", file=sys.stderr)
    ...
    with pymupdf.open(path) as doc:
        ...
        if num not in figs:
            ...
            return 1
        f = figs[num]
        pix = doc[f["page"]].get_pixmap(dpi=dpi, clip=f["rect"])
